...
<div class='body'>
  <div class='body-request' style="background-color:#CDC6C2;">
    ...
    <div class='buttons-section-bottom'>
                        <button type="cancel" class='reject-button' onclick="rechazar({{$organizacione->id}})" style="background-color: #dc3545;">Rechazar</button>                    </div>
                        <form id="myFormAceptar" action="(' {{ url('/aceptarOrg/'.$organizacione->id) }}')" method="post">
                        {{csrf_field()}}
            <button type="submit" style="background-color: #103965">Aceptar</button>
        </form>
                </div>
                </div>
            </div>
